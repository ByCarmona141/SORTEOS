...
@section('content')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-lg">
        @forelse ($payments as $payment)
            ...
            <div class="bg-surface-container border border-surface-variant rounded-xl p-lg flex flex-col sm:flex-row gap-lg relative overflow-hidden">
                ...
                <div class="flex-1">
                    ...
                    <span class="inline-flex items-center gap-1.5 border px-2 py-1 rounded font-mono-label text-label-caps {{ $badge['classes'] }}">
                        ...
                        <button type="button" class="flex-1 bg-transparent text-on-surface border border-outline-variant hover:border-primary py-2 px-4 rounded-lg font-bold flex justify-center items-center gap-2 transition-colors"
                                data-modal-open="receipt-modal-{{ $payment->id }}">
                ...
                @if ($canReview)
                    <div class="flex flex-col gap-3 justify-center sm:w-40 border-t sm:border-t-0 sm:border-l border-surface-variant pt-4 sm:pt-0 sm:pl-lg">
                        ...
                        @if ($status !== 'rechazado')
                            <form action="{{ route('payment.reject', $payment) }}" method="POST" onsubmit="return confirm('¿Rechazar este pago?');">
                                @csrf
            ...
            {{-- Panel lateral con el comprobante, igual al diseño original --}}
            <div class="hidden fixed inset-0 z-50 flex justify-end bg-background/80 backdrop-blur-sm receipt-modal-backdrop" id="receipt-modal-{{ $payment->id }}">
                <div class="w-full max-w-md h-full bg-surface-container border-l border-surface-variant shadow-2xl flex flex-col">
                    <div class="p-lg border-b border-outline-variant flex justify-between items-center bg-surface-container-lowest">
                        ...
                        <button type="button" class="text-on-surface-variant hover:text-primary transition-colors" data-modal-close="receipt-modal-{{ $payment->id }}">
                        ...
                        <div class="bg-surface-container-lowest p-md rounded-xl border border-outline-variant">
                            @if ($payment->proof_image)
                                <img class="w-full h-auto rounded-lg object-contain"
                                     ...
                                     alt="Comprobante"
                                     onerror="this.replaceWith(Object.assign(document.createElement('p'), {className: 'text-error text-center py-8 text-sm', textContent: 'No se pudo cargar la imagen. ¿Corriste php artisan storage:link?'}))">
                            @else
        ...
        document.addEventListener('click', function (event) {
            ...
            if (openBtn) {
                ...
                if (modal) {
                    ...
                    console.error('No se encontró el modal con id:', openBtn.dataset.modalOpen);
                }
            }

            const closeBtn = event.target.closest('[data-modal-close]');
            if (closeBtn) {
                document.getElementById(closeBtn.dataset.modalClose)?.classList.add('hidden');
            }

            // Cierra el modal si le das clic al fondo oscuro (fuera de la tarjeta blanca)
